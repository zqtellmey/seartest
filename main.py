...
def main():
    ...
    with SB(**opts) as sb:
        try:
            ...
            # 2. 点击登录链接 (使用鲁棒性强的文本+属性联合定位)
            login_xpath = '//a[contains(@class, "nav-link") and contains(., "Login")]'
            ...
            # 点击 Continue (使用包含按钮文本与提交类型的鲁棒定位)
            sb.click('//button[@type="submit" and contains(., "Continue with email")]')
            print("[*] 已提交邮箱")
            ...
            # 点击最终登录 (使用包含按钮文本与提交类型的鲁棒定位)
            sb.click('//button[@type="submit" and contains(., "Login")]')
            print("[*] 正在执行最终登录...")
